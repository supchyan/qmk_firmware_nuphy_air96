...
static void react_point_playing(uint8_t step, uint8_t len, uint16_t *point) {
    ...
    r_temp = react_colour_lib[*point][0];
    g_temp = react_colour_lib[*point][1];
    b_temp = react_colour_lib[*point][2];

    // if(old_time != last_input_activity_time()) {
    //     *point = 0;
    //     cond = 1;
    //     old_time = last_input_activity_time();
    // }
    if(KEY_DOWN) {
        *point = 0;
        cond = 1;
...
static void side_react_mode_show(void) {
    ...
    if (0) {
        ...
        if (play_index == 0 && ++side_play_point >= LIGHT_COLOUR_MAX) side_play_point = 0;
