...
#define SIDE_REACT          0
#define SIDE_REACT_ANI      1
#define SIDE_REACT_TAIKO    2
#define SIDE_WAVE           3
#define SIDE_MIX            4
#define SIDE_STATIC         5
#define SIDE_BREATH         6
#define SIDE_OFF            7

#define LIGHT_COLOUR_MAX 5
#define SIDE_COLOUR_MAX  5
#define LIGHT_SPEED_MAX  4
#define SIDE_BLINK_LIGHT 128
#define SIDE_LINE  5
#define SIDE_INDEX 100

const uint8_t side_speed_table[8][5] = {
    [SIDE_REACT]  = {1, 5, 10, 25, 50},
    [SIDE_REACT_ANI]  = {1, 5, 10, 25, 50},
    [SIDE_REACT_TAIKO]  = {1, 5, 10, 25, 50},
...
uint8_t r_tempL, g_tempL, b_tempL;
uint8_t r_tempR, g_tempR, b_tempR;
...
static bool react_condition = 0;
static bool react_left_condition = 0;
static bool react_right_condition = 0;
...
static void react_point_playing(uint8_t step, uint8_t len, uint16_t *point) {
...
static void react_taiko_point_playing(uint8_t step, uint8_t len, uint16_t *left_point, uint16_t *right_point) {

    if(DRUM_LEFT_RIM || DRUM_LEFT_CENTRE) {
        *left_point = 0;
        react_left_condition = 1;
    }

    if(react_left_condition) {
        *left_point += step;
        if (*left_point >= len) {
            *left_point = 0;
            react_left_condition = 0;
        }
    }

    if(DRUM_RIGHT_RIM || DRUM_RIGHT_CENTRE) {
        *right_point = 0;
        react_right_condition = 1;
    }

    if(react_right_condition) {
        *right_point += step;
        if (*right_point >= len) {
            *right_point = 0;
            react_right_condition = 0;
        }
    }
}

static void react_ani_point_playing(uint8_t step, uint8_t len, uint16_t *point) {

    // this sets color animation euqals transition length [ len === REACT_TAB_LEN ]
    switch(side_colour) {
        case 0:
            r_temp = react_tr0_col[*point][0];
            g_temp = react_tr0_col[*point][1];
            b_temp = react_tr0_col[*point][2];
            break;

        case 1:
            r_temp = react_tr1_col[*point][0];
            g_temp = react_tr1_col[*point][1];
            b_temp = react_tr1_col[*point][2];
            break;

        case 2:
            r_temp = react_tr2_col[*point][0];
            g_temp = react_tr2_col[*point][1];
            b_temp = react_tr2_col[*point][2];
            break;
        case 3:
            r_temp = react_tr3_col[*point][0];
            g_temp = react_tr3_col[*point][1];
            b_temp = react_tr3_col[*point][2];
            break;
...
static void count_left_rgb_light(uint8_t light_temp) {
    uint16_t temp;

    temp   = (light_temp)*r_tempL + r_tempL;
    r_tempL = temp >> 8;

    temp   = (light_temp)*g_tempL + g_tempL;
    g_tempL = temp >> 8;

    temp   = (light_temp)*b_tempL + b_tempL;
    b_tempL = temp >> 8;
}
static void count_right_rgb_light(uint8_t light_temp) {
    uint16_t temp;

    temp   = (light_temp)*r_tempR + r_tempR;
    r_tempR = temp >> 8;

    temp   = (light_temp)*g_tempR + g_tempR;
    g_tempR = temp >> 8;

    temp   = (light_temp)*b_tempR + b_tempR;
    b_tempR = temp >> 8;
}
/**
 * @brief  side_react_mode_show
 */
static void side_react_mode_show(void) {
    static uint16_t play_index = 0;

    if (side_play_cnt <= side_speed_table[side_mode][side_speed]) return;

    else side_play_cnt -= side_speed_table[side_mode][side_speed];

    if (side_play_cnt > 20) side_play_cnt = 0;

    react_point_playing(1, REACT_TAB_LEN, &play_index);

    r_temp = colour_lib[side_colour][0];
    g_temp = colour_lib[side_colour][1];
    b_temp = colour_lib[side_colour][2];

    count_rgb_light(react_data_tab[play_index]);
 ...
 * @brief  side_react_mode_show
 */
static void side_react_ani_mode_show(void) {
    static uint16_t play_index = 0;
    ...
    react_ani_point_playing(1, REACT_TAB_LEN, &play_index);

    // if (0) { if (play_index == 0 && ++side_play_point >= LIGHT_COLOUR_MAX) side_play_point = 0; }
...
/**
 * @brief  side_taiko_mode_show
 */
static void side_react_taiko_mode_show(void) {
    static uint16_t left_index = 0;
    static uint16_t right_index = 0;
    ...
    react_taiko_point_playing(1, REACT_TAB_LEN, &left_index, &right_index);

    if(DRUM_LEFT_RIM) {
        r_tempL = react_taiko_col[1][0];
        g_tempL = react_taiko_col[1][1];
        b_tempL = react_taiko_col[1][2];
    }
    if(DRUM_LEFT_CENTRE) {
        r_tempL = react_taiko_col[0][0];
        g_tempL = react_taiko_col[0][1];
        b_tempL = react_taiko_col[0][2];
    }
    if(DRUM_RIGHT_CENTRE) {
        r_tempR = react_taiko_col[0][0];
        g_tempR = react_taiko_col[0][1];
        b_tempR = react_taiko_col[0][2];
    }
    if(DRUM_RIGHT_RIM) {
        r_tempR = react_taiko_col[1][0];
        g_tempR = react_taiko_col[1][1];
        b_tempR = react_taiko_col[1][2];
    }

    count_left_rgb_light(react_data_tab[left_index]);
    count_right_rgb_light(react_data_tab[right_index]);

    count_left_rgb_light(side_light_table[side_light]);
    count_right_rgb_light(side_light_table[side_light]);

    set_left_rgb(r_tempL, g_tempL, b_tempL);
    set_right_rgb(r_tempR, g_tempR, b_tempR);
}
/**
...
void m_side_led_show(void)
{
    ...
    switch (side_mode) {
        case SIDE_REACT:            side_react_mode_show();         break;
        case SIDE_REACT_ANI:        side_react_ani_mode_show();     break;
        case SIDE_REACT_TAIKO:      side_react_taiko_mode_show();   break;
